...
auto appendCallParameterInlayHints(const AnalysisResult& analysisResult, unsigned bufferId,
  ...
  auto tryHintCall = [&](const lesma::FuncCall* call, const lesma::Expression* receiver) -> void {
    ...
    // Prefer the typechecker-resolved callee so we still show hints when LSP type inference
    // cannot classify an argument (e.g. BinaryOp like `x - 1` is not handled by
    // resolveExpressionTypeAtOffset).
