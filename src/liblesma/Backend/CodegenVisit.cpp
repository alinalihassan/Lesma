...
Codegen::Codegen(
    ...
    Timer* performanceTimer, bool emitDebug, bool emitArcDebugArg, bool emitArcTraceArg,
...
auto Codegen::defineFunction(lesma::Value* value, const FuncDecl* node, Value* clsSymbol) -> void {
  ...
  AsyncCodegenStateSnapshot savedAsyncState = saveAsyncCodegenState();
  resetAsyncCodegenState();
  deferStack.emplace();
  pushDeferBaseline();
  ...
  if (node->getIsAsync()) {
    if (llvm::BasicBlock* cur = builder->GetInsertBlock();
      ...
      if (currentAsyncReturnPayloadType != nullptr &&
        ...
        emitImplicitAsyncVoidCompletion();
      } else if (cur->empty()) {
    ...
    finalizeAsyncCoroutine(f, *asyncBlocks);
  ...
  restoreAsyncCodegenState(savedAsyncState);
...
auto Codegen::defineLambdaFunction(lesma::Value* value, const LambdaExpr* node) -> void {
  ...
  if (scope != nullptr && specializationEnvs.contains(value)) {
    currentGenericTypes = specializationEnvs.at(value);
    std::unordered_map<SymbolTable*, SymbolTable*> clonedScopeMap;
    auto clonedRoot = scope->cloneSubtreeForCodegen(savedScope, clonedScopeMap);
    clonedRoot->remapValueBodyScopesForCodegenClone(clonedScopeMap);
    scope = savedScope->attachClonedChild(value->getName() + ".specialized", std::move(clonedRoot));
    codegenTemplateBodyScopeRemap = std::move(clonedScopeMap);
    for (const auto& [genericName, genericType] : currentGenericTypes) {
      if (genericType == nullptr) {
        continue;
  ...
  if (node->isExpressionBody()) {
    ...
    if (node->getIsAsync()) {
      emitCurrentAsyncReturnValue(node->getSpan(), result, "Async lambda body did not produce a value");
    } else if (rt != nullptr && rt->is(BaseType::TY_VOID)) {
      emitReleaseCurrentArcOwnedSlots();
      builder->CreateRetVoid();
    } else {
      if (rv == nullptr) {
    ...
    if (builder->GetInsertBlock()->getTerminator() == nullptr) {
      Type* rt = value->getType()->getReturnType();
      if (node->getIsAsync() && currentAsyncReturnPayloadType != nullptr &&
          currentAsyncReturnPayloadType->is(BaseType::TY_VOID)) {
        flushDeferredFramesForReturn();
        emitImplicitAsyncVoidCompletion();
      } else if (rt != nullptr && rt->is(BaseType::TY_VOID)) {
...
auto Codegen::saveAsyncCodegenState() const -> AsyncCodegenStateSnapshot {
  return AsyncCodegenStateSnapshot{
      .coroHandle = currentAsyncCoroHandle,
      .promisePtr = currentAsyncPromisePtr,
      .returnBlock = currentAsyncReturnBlock,
      .returnPayloadType = currentAsyncReturnPayloadType,
  };
}

auto Codegen::restoreAsyncCodegenState(const AsyncCodegenStateSnapshot& state) -> void {
  currentAsyncCoroHandle = state.coroHandle;
  currentAsyncPromisePtr = state.promisePtr;
  currentAsyncReturnBlock = state.returnBlock;
  currentAsyncReturnPayloadType = state.returnPayloadType;
}

auto Codegen::resetAsyncCodegenState() -> void {
...
auto Codegen::initializeAsyncCoroutine(llvm::Function* f, lesma::Type* callableReturnType,
                                       llvm::SMRange span, llvm::StringRef callableKind)
    -> AsyncCoroutineBlocks {
  currentAsyncReturnPayloadType = getAsyncTaskPayloadType(callableReturnType);
  if (currentAsyncReturnPayloadType == nullptr) {
    throw CodegenError(span, "Async {} is missing an internal task return type",
                       callableKind.str());
  }
  auto* promiseTy = getOrCreateAsyncPromiseLlvmType(currentAsyncReturnPayloadType);
  AsyncCoroutineBlocks blocks{
      .suspendBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.suspend", f),
      .resumeBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.resume", f),
      .cleanupBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.cleanup", f),
      .trapBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.trap", f),
      .dynAllocBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.dyn.alloc", f),
      .coroBeginBlock = llvm::BasicBlock::Create(theModule->getContext(), "async.coro.begin", f),
  };
  ...
  blocks.coroId =
      builder->CreateCall(coroIdFn,
                          {llvm::ConstantInt::get(builder->getInt32Ty(), 0), currentAsyncPromisePtr,
                           nullPtr, nullPtr},
                          "async.coro.id");
  ...
  coroMemPhi->addIncoming(coroMem, blocks.dynAllocBlock);
  ...
  auto* initialSwitch = builder->CreateSwitch(initialSuspend, blocks.suspendBlock, 2);
  initialSwitch->addCase(llvm::ConstantInt::get(builder->getInt8Ty(), 0), blocks.resumeBlock);
  initialSwitch->addCase(llvm::ConstantInt::get(builder->getInt8Ty(), 1), blocks.cleanupBlock);
  builder->SetInsertPoint(blocks.resumeBlock);
  return blocks;
}

auto Codegen::emitCurrentAsyncReadyFlag(lesma::Type* payloadType, bool isReady,
                                        llvm::StringRef name) -> void {
  auto* promiseTy = getOrCreateAsyncPromiseLlvmType(payloadType);
  llvm::Value* readyPtr = builder->CreateStructGEP(promiseTy, currentAsyncPromisePtr, 0, name);
  builder->CreateStore(llvm::ConstantInt::get(builder->getInt1Ty(), isReady), readyPtr);
}

auto Codegen::emitCurrentAsyncReturnValue(llvm::SMRange span, std::unique_ptr<lesma::Value>& value,
                                          llvm::StringRef missingValueMessage) -> void {
  if (currentAsyncReturnPayloadType == nullptr || currentAsyncPromisePtr == nullptr ||
      currentAsyncReturnBlock == nullptr) {
    throw CodegenError(span, "Internal error: async return outside initialized coroutine");
  }
  emitCurrentAsyncReadyFlag(currentAsyncReturnPayloadType, true, "async.promise.ready.ptr");
  if (currentAsyncReturnPayloadType->is(BaseType::TY_VOID)) {
    emitReleaseCurrentArcOwnedSlots();
    builder->CreateBr(currentAsyncReturnBlock);
    return;
  }
  if (value == nullptr || value->getType() == nullptr) {
    throw CodegenError(span, std::string(missingValueMessage));
  }
  if (!value->getType()->isEqual(currentAsyncReturnPayloadType)) {
    value = cast(span, value.get(), currentAsyncReturnPayloadType);
  }
  llvm::Value* payloadValue = value->getLlvmValue();
  bool storesFuncValuePair = value->getStoresFuncValuePair();
  ...
  if (payloadValue == nullptr) {
    throw CodegenError(span, "Async return value has no LLVM payload");
  }
  if (!value->getArcOwnedValue() && TypeUtils::containsArcManagedValue(currentAsyncReturnPayloadType)) {
    emitRetainLoadedValue(currentAsyncReturnPayloadType, payloadValue, storesFuncValuePair);
  }
  auto* promiseTy = getOrCreateAsyncPromiseLlvmType(currentAsyncReturnPayloadType);
...
auto Codegen::emitImplicitAsyncVoidCompletion() -> void {
  if (currentAsyncReturnPayloadType == nullptr || !currentAsyncReturnPayloadType->is(BaseType::TY_VOID)) {
    return;
  }
  emitCurrentAsyncReadyFlag(currentAsyncReturnPayloadType, true, "async.promise.ready");
  emitReleaseCurrentArcOwnedSlots();
  builder->CreateBr(currentAsyncReturnBlock);
}

auto Codegen::finalizeAsyncCoroutine(llvm::Function* f, const AsyncCoroutineBlocks& blocks) -> void {
  ...
  llvm::Value* finalSuspend =
      builder->CreateCall(finalSuspendFn,
                          ...
                          "async.final.suspend");
  auto* finalSwitch = builder->CreateSwitch(finalSuspend, blocks.suspendBlock, 2);
  finalSwitch->addCase(llvm::ConstantInt::get(builder->getInt8Ty(), 0), blocks.trapBlock);
  ...
  builder->CreateBr(blocks.suspendBlock);

  builder->SetInsertPoint(blocks.trapBlock);
  auto* trapFn = llvm::Intrinsic::getOrInsertDeclaration(theModule.get(), llvm::Intrinsic::trap, {});
  builder->CreateCall(trapFn, {});
  builder->CreateUnreachable();

  builder->SetInsertPoint(blocks.suspendBlock);
...
[[nodiscard]] auto typeContainsUnboundGenericImpl(lesma::Type* type,
  ...
  if (type->is(BaseType::TY_CLASS)) {
    if (type->isBuiltinTask()) {
      return typeContainsUnboundGenericImpl(type->getTaskPayloadType(), active);
    }
...
auto Codegen::visit(const VarDecl* node) -> void {
  ...
  if (existing != nullptr && existing->getLlvmValue() == nullptr) {
    ...
    if (node->isExported()) {
      ...
      existing->setCategory(ValueCategory::ADDRESSABLE_STORAGE);
      existing->setMutable(node->getMutability());
      registerModuleArcRoot(gv, storedType, filename + "::" + name,
                            existing->getStoresFuncValuePair());
...
auto Codegen::getOrCreateAnyTypeInfoGlobal(lesma::Type* type) -> llvm::GlobalVariable* {
  ...
  for (char ch : typeName) {
    globalName.push_back((std::isalnum(static_cast<unsigned char>(ch)) != 0) ? ch : '_');
  }
...
auto Codegen::emitAnyTypeInfoMatches(llvm::Value* typeInfo, lesma::Type* candidate,
  ...
  if (canonicalCandidate->isBuiltinStringClass()) {
    ...
    match = builder->CreateOr(
        match,
        builder->CreateICmpEQ(typeInfo, emitAnyTypeInfoPtr(cstrType), name + ".string.compat"),
        name + ".string.box");
    auto addBuiltinStringCompat = [this, &match, typeInfo, &name](Value* strSymbol) {
      ...
      match = builder->CreateOr(match,
                                builder->CreateICmpEQ(typeInfo,
                                                      emitAnyTypeInfoPtr(strSymbol->getType()),
...
auto Codegen::emitUnboxFromAny(llvm::SMRange span, lesma::Value* value, lesma::Type* targetType)
  ...
  if (canonicalTargetType->is(BaseType::TY_PTR) &&
      canonicalTargetType->getElementType() != nullptr &&
  ...
  if (canonicalTargetType->isBuiltinStringClass()) {
    ...
    auto* exactBlock = llvm::BasicBlock::Create(theModule->getContext(), "any.str.exact", parentFn);
    ...
    auto* mergeBlock = llvm::BasicBlock::Create(theModule->getContext(), "any.str.merge", parentFn);
...
auto Codegen::visit(const Return* node) -> void {
  ...
  if (lesma::Type* asyncPayloadType = getAsyncTaskPayloadType(currentFunction->getType()->getReturnType());
    ...
    node->getValue()->accept(*this);
    emitCurrentAsyncReturnValue(node->getSpan(), result, "Async return did not produce a value");
    return;
  }
  ...
  if (node->getValue() == nullptr) {
    ...
    if (actualReturnType == expectedReturnType) {
      ...
      builder->CreateRet(result->getLlvmValue());
               ...
               actualType->isEqual(declaredReturnType) && !actualReturnType->isPointerTy() &&
               !expectedReturnType->isPointerTy() &&
...
auto Codegen::visit(const Enum* node) -> void {
  ...
  for (unsigned i = 0; i < variants.size(); ++i) {
    ...
    declareOrDefineSyntheticEnumMethod(type, node, variants[i], i,
                                       SyntheticEnumMethodKind::CONSTRUCTOR);
  }
  selfSymbol = nullptr;
}

auto Codegen::emitEnumMonomorph(lesma::Type* specialized, const Enum* templateAst)
    -> lesma::Value* {
...
auto Codegen::declareOrDefineSyntheticEnumMethod(lesma::Type* enumType, const Enum* astNode,
  ...
  Value* existing = scope->lookupFunction(name, lookupParamTypes,
                                          FunctionLookupKind::OVERLOAD_IDENTITY, nullptr, enumType);
...
auto Codegen::defineSyntheticEnumMethod(const SyntheticEnumMethodBody& body) -> void {
  if (body.symbol == nullptr || body.enumType == nullptr ||
      body.symbol->getLlvmValue() == nullptr) {
    return;
  }
  ...
  case SyntheticEnumMethodKind::CONSTRUCTOR: {
    ...
    auto constructed = emitEnumConstructValue(variant->getDeclarationSpan(), body.enumType,
                                              body.variantIndex, payloadValues);
...
auto Codegen::visit(const LambdaExpr* node) -> void {
  ...
  if (node->isExpressionBody()) {
    ...
    flushDeferredFramesForReturn();
    if (node->getIsAsync()) {
      emitCurrentAsyncReturnValue(node->getSpan(), result,
                                  "Async lambda expression body did not produce a value");
    } else if (retType->is(BaseType::TY_VOID)) {
      emitReleaseCurrentArcOwnedSlots();
      builder->CreateRetVoid();
    } else {
      if (rv == nullptr) {
  ...
  builder->CreateStore(
      llvm::ConstantInt::get(builder->getInt1Ty(), static_cast<uint64_t>(!caps.empty())),
...
auto Codegen::visit(const BinaryOp* node) -> void {
  ...
  auto emitNullEquality = [this](lesma::Value* lhs, lesma::Value* rhs,
    ...
    if ((lhsTy->is(BaseType::TY_UNION) && rhsNull) || (rhsTy->is(BaseType::TY_UNION) && lhsNull)) {
      return nullptr;
    }
    if (!lhsNull && !rhsNull) {
      return nullptr;
...
auto Codegen::visit(const IsOp* node) -> void {
  ...
  if (leftType != nullptr && leftType->is(BaseType::TY_UNION)) {
    ...
    llvm::Value* cmp =
        idxOpt.has_value()
            ? builder->CreateICmpEQ(tagVal,
                                    llvm::ConstantInt::get(tagTy, static_cast<uint64_t>(*idxOpt)))
            : builder->getFalse();
    if (anyIdxOpt.has_value()) {
      llvm::Value* anyTagCmp = builder->CreateICmpEQ(
          tagVal, llvm::ConstantInt::get(tagTy, static_cast<uint64_t>(*anyIdxOpt)));
      ...
      if (rightType->is(BaseType::TY_ANY)) {
        ...
        builder->SetInsertPoint(activeBlock);
        llvm::Value* anyPayload =
            emitUnionPayloadLoadFromSlot(unionPtr, leftType, members[*anyIdxOpt]);
...
auto Codegen::visit(const MatchExpr* node) -> void {
  ...
  if (node->getUsesEnumDispatch()) {
    ...
    auto emitValueMatchEquals = [&](Value* leftValue, Value* rightValue,
                                    Type* rightType) -> llvm::Value* {
      ...
      throw CodegenError(node->getSpan(), "Value match is not yet supported for type {}",
    ...
    if (builder->GetInsertBlock() == currentTestBlock &&
        currentTestBlock->getTerminator() == nullptr) {
      builder->SetInsertPoint(currentTestBlock);
      builder->CreateUnreachable();
    }
  }
...
auto Codegen::cast(llvm::SMRange span, lesma::Value* val, lesma::Type* type)
  ...
  if (type != nullptr && type->is(BaseType::TY_UNION) && val != nullptr &&
    ...
    auto nominalTemplateName = [this](lesma::Type* nominal) -> std::string {
      ...
      if (auto it = specializedClassTemplateOf.find(nominal);
          it != specializedClassTemplateOf.end()) {
        nominal = it->second;
      }
    ...
    auto unionArmAccepts = [this, &nominalTemplateName](auto&& self, lesma::Type* from,
      ...
      if (to->is(BaseType::TY_UNION)) {
        return std::ranges::any_of(to->getUnionMembers(), [&self, &from](Type* member) -> bool {
          return self(self, from, member);
        });
      }
      if (from->is(BaseType::TY_UNION)) {
        return std::ranges::all_of(from->getUnionMembers(), [&self, &to](Type* member) -> bool {
          return self(self, member, to);
        });
      }
      if (from->isOneOf({BaseType::TY_CLASS, BaseType::TY_ENUM}) &&
          to->isOneOf({BaseType::TY_CLASS, BaseType::TY_ENUM})) {
        Type* fromTemplate = from;
        if (auto it = specializedClassTemplateOf.find(from);
            it != specializedClassTemplateOf.end()) {
        ...
        if (!fromTemplate->isEqual(toTemplate)) {
          ...
          if (fromTemplateName.empty() || toTemplateName.empty() ||
              fromTemplateName != toTemplateName) {
            return false;
          }
        }
      ...
      if (canWidenWithMatchingArms) {
        ...
        for (auto const& pr : phiIncomings) {
          if (phi->getType() != pr.first->getType()) {
            throw CodegenError(span, "Internal error: union widen PHI type mismatch in function {}",
                               currentFunction != nullptr ? std::string(currentFunction->getName())
                                                          : "<none>");
          }
...
auto Codegen::callNamedFunction(
  ...
  const LambdaExpr* originLambda = symbol->getOriginLambdaExpr();
  Value* originLambdaSymbol = originLambda != nullptr ? originLambda->getResolvedSymbol() : nullptr;
  const bool genericLambdaNeedsSpecialize =
      originLambdaSymbol != nullptr && originLambdaSymbol->getType() != nullptr &&
      !originLambdaSymbol->getType()->getGenericParams().empty();
  if (symbol->getLlvmValue() == nullptr || genericLambdaNeedsSpecialize) {
    ...
    } else if (originLambda != nullptr) {
      const LambdaExpr* lamNode = originLambda;
  ...
  if (typecheckCalleeFallback != nullptr && typecheckCalleeFallback->getType() != nullptr &&
      typecheckCalleeFallback->getType()->is(BaseType::TY_FUNCTION) &&
      !typeContainsUnboundGeneric(typecheckCalleeFallback->getType()) &&
      (callableLesmaType == nullptr || typeContainsUnboundGeneric(callableLesmaType))) {
    callableLesmaType = typecheckCalleeFallback->getType();
  }
  ...
  if (callableLesmaType->is(BaseType::TY_CLASS)) {
    ...
    if (symbol->getStoresFuncValuePair()) {
      ...
      if (symbol->getLlvmValue() == nullptr) {
        ...
        if (!symbol->getMangledName().empty() && !declaredPath.empty() &&
            declaredPath != currentPath && callableLesmaType->getLlvmType() != nullptr) {
...
auto Codegen::callMethodByName(llvm::SMRange span, lesma::Value* receiver,
  ...
  auto* cls =
      receiverType->is(BaseType::TY_CLASS) ? lookupClassStructSymbol(receiverType) : nullptr;
  ...
  auto preferImportedMethodWithBody = [&](lesma::Value* candidate,
                                          Type* declaredInClass = nullptr) -> lesma::Value* {
  ...
  if (staticCall) {
    ...
    if (emittedEnumMonomorph && receiverType->is(BaseType::TY_ENUM)) {
      if (Value* specializedMethod =
              scope->lookupFunction(methodName, paramTypes, FunctionLookupKind::VALUE, nullptr,
    ...
    if (directMethod == nullptr || directMethod->getLlvmValue() == nullptr) {
      directMethod = scope->lookupFunction(methodName, paramTypes, FunctionLookupKind::VALUE,
                                           nullptr, requiredDeclaredInClass, requiredGenericArity);
  ...
  if (directMethod != nullptr && directMethod->getLlvmValue() != nullptr) {
    ...
    try {
      ...
      const bool canUseVirtual = receiverType->is(BaseType::TY_CLASS) && methodName != "new" &&
                                 vtableSlot != ~0U && !vtOrder.empty() &&
...
auto Codegen::genFuncCall(const FuncCall* node, const std::vector<lesma::Value*>& extraParams = {})
  ...
  if (Type* contextualEnum = node->getContextualEnumMonomorph(); contextualEnum != nullptr) {
    ...
    if (currentFunction != nullptr && currentFunction->getType() != nullptr &&
      ...
      if (auto it = specializedClassTemplateOf.find(declaredShape);
          it != specializedClassTemplateOf.end()) {
    ...
    if (enumType->is(BaseType::TY_ENUM)) {
      ...
      if (auto it = specializedClassTemplateOf.find(enumType);
          it != specializedClassTemplateOf.end()) {
      ...
      if (variantIndex >= 0) {
        ...
        return emitEnumConstructValue(node->getSpan(), enumType,
                                      static_cast<unsigned>(variantIndex), payloadValues);
      }
    }
  }
