 ...
 */
const getPivotItemKey = (item?: PivotItem | React.ReactElement<{ itemKey?: string }>): string | undefined => {
  if (!item) {
    return undefined
  }
  const fromProps = item.props.itemKey
  const fromKey = React.isValidElement(item) && item.key != null ? String(item.key) : undefined
  return fromProps ?? fromKey
}
...
const getInitialKey = (props: Pick<Props, 'children' | 'defaultSelectedKey'>): string => {
  ...
  const first = React.Children.toArray(props.children)[0] as
    | React.ReactElement<{ itemKey?: string }>
    | undefined
  return getPivotItemKey(first) ?? defaultSelectedKey
}
