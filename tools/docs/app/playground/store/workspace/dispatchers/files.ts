...
import type { Action } from '@/playground/store/actions/actions'
import {
  newNotificationId,
  newAddNotificationAction,
  newAddNotificationsAction,
} from '@/playground/store/notifications/actions'
import { type Notification, NotificationType } from '@/playground/store/notifications/state'
...
const IMPORTABLE_EXTENSIONS = new Set(['.les', '.txt', '.json'])
...
export const dispatchImportFile = (files: FileList) => async (dispatch: DispatchFn, getState: StateProvider) => {
  ...
  if (ignoredFiles > 0) {
    errors.push({
      ...
      description: `${ignoredFiles} file(s) skipped: use .les, .txt, or .json.`,
      canDismiss: true,
    })
  }
